...
const APR = (totalStaked: BigNumber, rewardPerBlock: BigNumber): number => {
  ...
  const EMISSION_VALUE_PER_YEAR = rewardPerBlock.mul(
    Math.floor(100 * SECONDS_PER_YEAR)
  );
...
function APRWidget({
  ...
  let formattedETHAPR =
    APR(totalStaked, ethPerSecond.mul(Math.round(ethPrice * 100))) /
    (tndPrice * 100);
    ...
    >
      ...
      >
        <span className="text-sm md:text-base">
          {(formattedAPR + formattedETHAPR).toPrecision(3) + "%"}
        </span>
          ...
          <div className="relative z-11 leading-none whitespace-no-wrap shadow-lg w-[242px] panel-custom !rounded-[10px]">
            <div className="w-full h-full bg-[#181D1B] shadow-lg rounded-[10px] p-[14px] pr-[16px] pl-[14px] pb-[15px]">
              ...
              <p className="text-[#818987] text-xs  text-left leading-[17px]">
                APRs are updated weekly on Wednesday and will depend on the fees
...
export function displayTNDInUSD(amount: BigNumber, tndPrice: number): string {
  ...
  return toCryptoString(
    formatUnits(TND_VALUE_IN_CENTS, TND.TND_DECIMALS + 2),
    4
  );
}
...
function displayTNDWithUSD(amount?: BigNumber, price?: number | null): string {
  if (!amount) return "?";
  return `${displayTND(amount)} ($${
    price ? displayTNDInUSD(amount, price) : "?"
  })`;
}
...
export default function EarnContent(): JSX.Element {
  ...
  const APRRow = data && (
    <Row
      ...
      right={
        <APRWidget
          totalStaked={data.totalESTNDStaked.add(data.totalTNDStaked)}
          ethPerSecond={data.ethEmissionsPerSecond}
          rewardPerSecond={data.emissionsPerBlock}
        />
      }
    />
  );
      ...
      >
        ...
        {currentModal === "unstake" && data && (
          <Modal
            ...
            totalBonusPoints={data.stakedBonusPoints.add(
              data.claimableBonusPoints
            )}
          />
        )}

        {currentModal === "stakeESTND" && (
          <Modal
            closeModal={closeModal}
      ...
      <div className="c focus:outline-none mt-[30px] mb-[60px] md:mb-[100px] font-nova">
        ...
        <div tabIndex={0} className="max-w-[1080px] my-o mx-auto">
          ...
          <p className="md:text-base md:leading-[22px] text-sm leading-5 mb-[31px] text-[#ADB5B3]">
            ...
            to learn more.
            <br />
            {data && (data.stakedTND.gt(0) || data.stakedESTND.gt(0)) && (
              <span>
                You are earning rewards with <br />
                {displayTND(data.stakedTND)} TND, {displayTND(data.stakedESTND)}{" "}
                esTND, and {displayTND(data.stakedBonusPoints)} Multiplier
                Points.
              </span>
            )}
          <br/><br/>
            ...
            <div tabIndex={0} className="panel-custom">
              ...
              <div className="px-[15px] pt-[20px] pb-[16.9px] md:px-[30px] md:pt-[24px] md:pb-[30px] text-sm leading-5 md:text-base md:leading-[22px]">
                      ...
                      <>
                        ...
                        >
                          <span className="text-sm md:text-base">
                            $
                            {data && tndPrice
                              ? displayTNDInUSD(data?.claimableESTND, tndPrice)
                              : "?"}
                          </span>
                          <div className="hidden z-10 flex-col absolute right-[-5px] bottom-[18px] items-center group-hover:flex group-focus:flex rounded-[10px]">
                            <div className="relative z-11 leading-none whitespace-no-wrap shadow-lg w-[242px] panel-custom !rounded-[10px]">
                        ...
                        >
                          {data && (
                            <span className="text-sm md:text-base">
                              ...
                              {data?.stakedBonusPoints &&
                              data?.stakedTND.add(data?.stakedESTND).gt(0)
                                ? `${data.stakedBonusPoints
                                    .mul(100)
                                    .div(data.stakedTND.add(data.stakedESTND))
                                    .toNumber()
                                    .toPrecision(3)}%`
                                : "0.00%"}
                            </span>
                              ...
                              <div className="w-full h-full bg-[#181D1B] shadow-lg rounded-[10px] p-[14px] pr-[16px] pl-[14px] pb-[15px] text-[#818987] text-start">
                                ...
                                data?.stakedTND.add(data?.stakedESTND).gt(0)
                                  ? ` ${data.stakedBonusPoints
                                      ...
                                      .toPrecision(3)} % `
                                  : " 0.00% "}
                                more TND rewards using{" "}
                                {data
                                  ? displayTND(data?.stakedBonusPoints)
                                  : " 0.00 "}
                                &nbsp; Staked Multiplier Points. <br />
                                <br />
                                Use the "Compound" button to stake your
                                Multiplier Points.
                              </div>
                            </div>
                            <div className="custom__arrow__tooltip relative right-[-95px] top-[-6px] z-[11] !mt-[0] !border-none w-3 h-3 rotate-45 bg-[#181D1B]"></div>
                ...
                <div className="flex  flex-col gap-y-[12px] md:gap-y-[15px] pt-[39px] md:pt-[24px]">
                  <Row
                    left="Total Staked"
                    amount={data?.totalTNDStaked}
                    symbol="TND"
                  />
                  <Row
                    left="Total Supply"
                    amount={data?.totalTNDSupply}
                    symbol="TND"
                  />
                </div>
            ...
            <div tabIndex={0} className="panel-custom">
              ...
              <div className="px-[15px] pt-[20px] pb-[15px] md:px-[30px] md:pt-[23px] md:pb-[30px] text-sm leading-5 md:text-base md:leading-[22px]">
                <div className="flex flex-col gap-y-[12px] md:gap-y-[15px]">
                  <Row
                    left="ETH"
                    right={displayETHWithUSD(
                      data?.claimableFees ?? BigNumber.from(0),
                      Math.round(ethPrice)
                    )}
                  />
                  ...
                  <Row
                    ...
                    right={
                      <>
                        ...
                        >
                          <span className="text-sm md:text-base">
                            {data ? displayTND(data.claimableBonusPoints) : ""}
                          </span>
                          <div
                            className={`${
                  ...
                  >
                    ...
                    >
                      <span className="text-sm md:text-base">
                        {data ? displayTND(data.stakedBonusPoints) : ""}
                      </span>
                      <div
                        className={`${
                  ...
                  >
                    ...
                    <div className="text-sm md:text-base leading-[17px]">
                      ...
                      {tndPrice &&
                        ...
                        displayTNDInUSD(
                          data.claimableESTND
                            .add(data.claimableTND)
                            .add(
                              data.claimableFees
                                .mul(Math.round((ethPrice / tndPrice) * 100))
                                .div(100)
                            ),
                          tndPrice
                        )}
            ...
            <div tabIndex={0} className="panel-custom mt-[31px]">
              ...
              <div className="px-[15px] pt-[20px] pb-[15.9px] md:px-[30px] md:pt-[23px] md:pb-[30px] text-sm leading-5 md:text-base md:leading-[22px]">
                <div className="border-[#282C2B] border-b-[1px] flex flex-col gap-y-[12px] md:gap-y-[15px] pb-[19px] md:pb-[23px] ">
                  ...
                  <Row
                    left="Wallet"
                    amount={data?.esTNDBalance}
                    symbol="esTND"
                  />
                  <Row
                    left="Staked"
                    amount={data?.stakedESTND}
                    symbol="esTND"
                  />
                </div>
                ...
                <div className="border-[#282C2B]  border-b-[1px] flex flex-col gap-y-[12px] md:gap-y-[15px] pt-[13px] pb-[20px] md:pt-[24px] md:pb-[23px] ">
                  <Row
                    left="Total Staked"
                    amount={data?.totalESTNDStaked}
                    symbol="esTND"
                  />
                  <Row
                    left="Total Supply"
                    amount={data?.totalESTNDSupply}
                    symbol="esTND"
                  />
                </div>
