...
export default function Modal({
  ...
  const [isEnabled, setIsEnabled] = useState<boolean>(
    (symbol === "TND" && sTNDAllowance?.gt(0)) || symbol === "esTND"
      ? true
      : false
  );
  ...
  const onChange = () => {
    ...
    try {
      var value = parseUnits(
        inputEl.current.value.replace(/,/g, ""),
        tnd.TND_DECIMALS
      );
    } catch (e) {
    ...
    if (
      ...
      symbol === "TND" &&
      sTNDAllowance?.gt(0) &&
      value.gt(sTNDAllowance) &&
      value.lte(balance)
    ) {
      setValidationDetail("Enable");
      setIsEnabled(false);
    } else {
      setIsEnabled(true);
    ...
    if (
      ...
      let downAmount =
        (parseFloat(formatUnits(totalBonusPoints, tnd.TND_DECIMALS)) *
          withdrawAmount) /
        staked;
      setWillLose(
        `You will lose ${downAmount.toPrecision(4)} (${(
          (100 * withdrawAmount) /
          staked
        ).toPrecision(4)}%)
      ...
      <div className="flex flex-col justify-center items-center overflow-hidden font-space min-h-[70px] h-[70px] pt-[96px] box-content">
        ...
        <Max
          ...
          updateValue={() => {
            ...
            if (inputEl.current)
              inputEl.current.value = formatUnits(balance, tokenDecimals);
            onChange();
          }}
