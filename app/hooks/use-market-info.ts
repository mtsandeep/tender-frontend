...
`);
            ...
            market.totalBorrowersCount = response.accountCTokens.filter(
                (account: { totalUnderlyingBorrowed: number; }) => account.totalUnderlyingBorrowed > 0
            ).length;
            market.totalSuppliersCount = response.accountCTokens.filter(
                (account: { totalUnderlyingSupplied: number; }) => account.totalUnderlyingSupplied > 0
            ).length;
            ...
            Object.keys(response).map(key => parseInt(key.substring(1)))
                .sort((a, b) => a - b).forEach((key) => {
                // @ts-ignore
